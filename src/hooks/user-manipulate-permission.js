...
module.exports = function (options = {}) {
  return async context => {
    ...
    case ROLES.supervisor :
      switch(context.method) {    // 定义visitor, student,
      case 'patch' :              // teacher， supervisor用户对用户数据的操作权限
      case 'update' :
      case 'remove' :
        if(context.id === null) {
          throw new Error('You don\'t have permission to modify multiple user data.');
        }
        if(context.id !== user._id) {
          throw new Error('You don\'t have permission to modify other user\'s data.');
        }
        break;
      default :
        break;
